...
movingwin = [0.5 0.005]; % movingwin = [winsize winstep] [0.5 0.02]
...
colormap turbo
%caxis([0.1e-5 2.5e-5]); % For 30-100 Hz
%caxis([-60 -45]); % For 30-100 Hz DB
caxis([-42 -30]); % For 1-8 Hz DB
% caxis([-50 -35]); % For 8-30 Hz DB
% caxis([-57 -45]); % For 30-80 Hz DB
xlabel("");
ylabel("");
title("");
saveas(gcf,fig_name);
...
signal_all = cell2mat(all_data);
signal_all = signal_all(1,:);
[S,f,Serr]=mtspecgramc(signal_all, movingwin, params);
f_t = f';
colormap turbo;
plot_matrix(S,t,f,'l');
